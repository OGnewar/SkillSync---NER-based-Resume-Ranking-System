...
def validateYears(exp_years):
    ...
    valid_formats = [
        ...
        r"^(?:At least|Minimum|More than|Over|Up to|Approximately|Around)\s+\d+\s*(?:years?|yrs?)$",  # "At least 4 years", "More than 3 years"
        r"^(?:Between|From)\s+\d+\s*(?:years?|yrs?)\s*(?:and|to)\s*\d+\s*(?:years?|yrs?)$",  # "Between 3 and 5 years", "From 2 to 4 years"
        r"^\d+\s*(?:to|-)\s*\d+\s*(?:years?|yrs?)$",  # "3 to 5 years", "2-4 yrs"
    ]
    
    valid_exp = []
    for exp in exp_years:
        if any(re.match(pattern, exp, re.IGNORECASE) for pattern in valid_formats):
